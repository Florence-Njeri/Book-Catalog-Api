...
RSpec.describe "Books API", type: :request do
   # initialize test data (using the factory bot)
  ...
  describe 'POST /books' do
    # valid payload
    let(:valid_attributes) { { title: 'Lies of Locke Lamora', author: 'Lynn Flinch', rating: 4.9 } }

    context 'when the request is valid' do
      before { post '/books', params: valid_attributes }

      it 'creates a book' do
        expect(json['title']).to eq('Lies of Locke Lamora')
      end
    ...
    context 'when the request is invalid' do
      before { post '/books', params: { title: 'Kintu', author: 'Jennifer Makumbi'} }
   ...
   describe 'PUT /books/:id' do
    let(:valid_attributes) { { title: 'Lies of Locke Lamora' } }
